const BASE_FEEDS = [
  ...
  { source: "Google News", topicKey: "mobility", query: "EV 自動車 企業 投資 OR 自動運転" },
  { source: "Google News", topicKey: "ai", query: "生成AI 企業 導入 業務効率化" },
  { source: "Google News", topicKey: "finance", query: "金利 企業 影響" },
  { source: "Google News", topicKey: "semiconductor", query: "半導体 投資 企業 サプライチェーン" },
  { source: "Google News", topicKey: "media", query: "サブスク 値上げ 動画配信" },
  { source: "Google News", topicKey: "creator", query: "フリーランス 取引条件 企業" },
  { source: "Google News", topicKey: "retail", query: "小売 物流 店舗 価格戦略 企業" },
  { source: "Google News", topicKey: "operations", query: "人手不足 省人化 物流 オペレーション 企業" },
  { source: "Google News", topicKey: "public", query: "経産省 白書 産業 企業 投資" }
];
...
function buildFeeds(searchQuery = "") {
  ...
  let feeds = BASE_FEEDS.map((feed) => ({ ...feed, url: feed.url || googleNewsUrl(feed.query) }));
  if (query) {
    feeds.unshift({
      source: "Google News",
      topicKey: inferTopicKey(query),
...
function parseItems(xml, source, topicKey = "") {
  ...
  return blocks.map((block) => {
    ...
    return { title, url, source, publishedAt, ageDays: ageDays(publishedRaw), theme: classifyTheme(title), material: classifyMaterial(title), topicKey: topicKey || inferTopicKey(title) };
  }).filter((item) => item.title && item.url && item.ageDays <= 7);
}

exports.handler = async (event) => {
  try {
    const feeds = buildFeeds(event.queryStringParameters?.q || "");
    const settled = await Promise.allSettled(
      feeds.map(async (feed) => {
        const response = await fetch(feed.url, { headers: { "user-agent": "CaseMaterialDesk/1.0" } });
    ...
    return {
      ...
      body: JSON.stringify({ articles: [], error: "failed_to_fetch_articles" })
    };
  }
};
